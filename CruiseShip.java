...
public class CruiseShip 
{
    ...
    public CruiseShip(String boatName, String departPort, Calendar departDate, 
            String arrivalPort, Calendar arrivalDate, float cost) {
        ...
        if(arrivalPort.equalsIgnoreCase(departPort))
            throw new IllegalArgumentException("CruiseShip constructor: ARRIVAL "
                    + "PORT "+ arrivalPort+" IS EQUAL TO DEPART PORT "+departPort);
        if(departDate.compareTo(arrivalDate)>0)
            throw new IllegalArgumentException("CruiseShip constructor: ARRIVAL "
                    + "DATE "+arrivalDate+" LATER THAN DEPART DATE "+departDate);
    }
    ...
    {   fmt.setCalendar(departDate);
        ...
        String s = ">>>>> BOAT: "+ boatName +". COST: $" + cost + " <<<<<< " +
            "\nLEAVING FROM " + departPort + " AT " + departFormatted +
            "\nARRIVING IN " + arrivalPort + " AT " + arrivalFormatted;
        return s;
    }
    @Override
